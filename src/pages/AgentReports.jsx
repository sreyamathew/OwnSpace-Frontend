...
const AgentReports = () => {
    ...
    useEffect(() => {
        const fetchReports = async () => {
            try {
                ...
                setError('Failed to load your sales reports. Please check your connection.');
                setLoading(false);
            }
        };
    ...
    const generateInsights = () => {
        if (!stats || !locationData.length) return ["Analyzing your sales data..."];

        const insights = [];
        if (stats.totalSold > 0) {
            insights.push(`You generated total revenue of ₹${(stats.totalRevenue / 10000000).toFixed(2)}Cr from ${stats.totalSold} properties.`);
        }

        if (locationData.length > 0) {
            const topLoc = locationData[0];
            insights.push(`${topLoc.name} is your top-performing location with ₹${(topLoc.totalSales / 100000).toFixed(1)}L in sales.`);
        }

        if (stats.highRiskCount > 0) {
            insights.push(`Notice: ${stats.highRiskCount} high-risk sales in your portfolio. review pricing strategies.`);
        } else {
            insights.push("Excellent work! All your recent sales align well with AI predicted values.");
        }
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-gray-50">
                <div className="flex flex-col items-center">
                    ...
                    <p className="text-gray-600 font-medium">Generating Your Performance Reports...</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-xl font-bold text-gray-900">Your Sales Performance</span>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            <div className="relative">
                                ...
                                >
                                    <div className="w-9 h-9 bg-green-600 rounded-full flex items-center justify-center text-white font-bold ring-2 ring-white shadow-sm">
                                ...
                                {profileDropdownOpen && (
                                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 z-50 py-2">
                                        ...
                                        <button onClick={() => navigate('/agent/profile')} className="flex items-center space-x-3 w-full px-4 py-2 text-sm text-gray-700 hover:bg-green-50 transition-colors">
                                            <User size={16} /> <span>Profile</span>
                                        </button>
                                        <button onClick={() => navigate('/agent/settings')} className="flex items-center space-x-3 w-full px-4 py-2 text-sm text-gray-700 hover:bg-green-50 transition-colors">
                ...
                <main className="p-8">
                    ...
                    <div className="mb-10 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                        <div>
                            ...
                            <p className="text-gray-500 mt-1 font-medium">Tracking your sales performance and AI-driven insights</p>
                        </div>
                    ...
                    {error ? (
                        <div className="p-12 text-center bg-white border border-red-100 rounded-2xl shadow-sm">
                            ...
                            <p className="text-gray-500 mb-6 font-medium">We encountered a problem while fetching your sales data.</p>
                            <button
                                onClick={() => window.location.reload()}
                                className="px-6 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 shadow-md transition-all active:scale-95"
                            >
                                Try Again
                            </button>
                        </div>
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
                                ...
                                <div className="lg:col-span-2 space-y-8">
                                    <div className="bg-white p-7 rounded-2xl shadow-md border border-gray-100">
                                        <div className="flex items-center justify-between mb-6">
                                            <h3 className="text-lg font-bold text-gray-800">Sales by City</h3>
                                            <button className="text-green-600 text-sm font-bold hover:underline">View My Areas</button>
                                        </div>
                                        <div className="h-80">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <BarChart data={locationData}>
                                                    ...
                                                    />
                                                    <Bar dataKey="totalSales" name="Sales Value" fill="#10B981" radius={[6, 6, 0, 0]} barSize={40} />
                                                </BarChart>
                                            </ResponsiveContainer>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white p-7 rounded-2xl shadow-md border border-gray-100">
                                        <div className="flex items-center justify-between mb-6">
                                            <h3 className="text-lg font-bold text-gray-800">My Sales Trend</h3>
                                        ...
                                        <div className="h-80">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <LineChart data={trendData}>
                                                    ...
                                                    <Line type="monotone" dataKey="count" name="Properties You Sold" stroke="#10B981" strokeWidth={4} dot={{ r: 6, fill: '#10B981', strokeWidth: 3, stroke: '#fff' }} activeDot={{ r: 8, strokeWidth: 0 }} />
                                                </LineChart>
                                            </ResponsiveContainer>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-8">
                                    <div className="bg-white p-7 rounded-2xl shadow-md border border-gray-100 h-full flex flex-col">
                                        <h3 className="text-lg font-bold text-gray-800 mb-6">Client Risk Profile</h3>
                                            ...
                                            <div className="space-y-4">
                                                {generateInsights().map((insight, idx) => (
                                                    <div key={idx} className="flex gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100 hover:bg-white hover:shadow-sm transition-all group">
                                                        <Info className="text-green-500 shrink-0 mt-0.5 group-hover:scale-110 transition-transform" size={16} />
                            ...
                            <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden mb-10">
                                <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                                    <h3 className="text-lg font-bold text-gray-800">My Recently Sold Properties</h3>
                                    <div className="flex items-center gap-4">
                                        <div className="relative">
                                            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-gray-400" />
                                            <input type="text" placeholder="Search my sales..." className="pl-9 pr-4 py-1.5 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-green-500 outline-none w-64" />
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="bg-gray-50">
                                                ...
                                                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Sale Price</th>
                                                <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">AI Estimate</th>
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {soldList.length > 0 ? soldList.map((item) => (
                                                <tr key={item._id} className="hover:bg-green-50/30 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <div className="text-sm font-bold text-green-600">₹{(item.soldPrice || item.price).toLocaleString()}</div>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <tr>
                                                    <td colSpan="6" className="px-6 py-12 text-center text-gray-500 font-medium italic">You haven't recorded any sold properties yet.</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
